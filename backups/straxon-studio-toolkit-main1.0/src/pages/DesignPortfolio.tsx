...
const designCategories = [
  {
    ...
    icon: PenTool,
    items: [
      { name: "Tech Startup Logo Suite", tools: "Illustrator, Figma", description: "Complete logo system with variations for a tech startup including horizontal, stacked, and icon-only versions. Delivered brand guidelines with color palette and typography." },
      { name: "Corporate Identity Marks", tools: "Illustrator, Photoshop", description: "Professional logo marks for corporate clients with multiple applications across stationery, digital, and signage." },
      { name: "Brand Monogram Collection", tools: "Illustrator", description: "Elegant monogram designs for luxury brands with gold foil and embossing-ready vector files." },
    ],
  },
  {
    ...
    icon: Image,
    items: [
      { name: "Social Media Campaign", tools: "Photoshop, Canva", description: "Eye-catching social media graphics and templates optimized for Instagram, LinkedIn, and Twitter with consistent branding." },
      { name: "Marketing Collateral Set", tools: "InDesign, Illustrator", description: "Brochures, flyers, and presentation decks with cohesive visual language and print-ready files." },
      { name: "Digital Ad Creatives", tools: "Photoshop, After Effects", description: "Animated and static ad banners for campaigns across Google Display Network and social platforms." },
    ],
  },
  {
    title: "UI/UX Design",
    icon: Layout,
    items: [
      { name: "SaaS Dashboard Design", tools: "Figma, Framer", description: "Data-rich analytics dashboard with clean UX, interactive charts, and comprehensive component library." },
      { name: "Mobile App Interface", tools: "Figma, Principle", description: "Mobile-first app design with micro-interactions, gesture-based navigation, and accessibility compliance." },
      { name: "E-commerce Experience", tools: "Adobe XD, Figma", description: "Complete e-commerce UI with checkout flows, product galleries, and personalized recommendation panels." },
    ],
  },
  {
    ...
    icon: Layers,
    items: [
      { name: "Complete Brand Book", tools: "InDesign, Illustrator", description: "Full brand guidelines with typography systems, color palette, iconography, and usage examples across all media." },
      { name: "Visual Identity System", tools: "Figma, Illustrator", description: "Comprehensive visual identity with applications across business cards, letterheads, social media, and merchandise." },
      { name: "Packaging Design", tools: "Illustrator, Photoshop", description: "Product packaging with premium finish concepts, 3D mockups, and production-ready dielines." },
    ],
  },
];
...
const DesignPortfolio = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<{ name: string; tools: string; description: string; category: string } | null>(null);
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {category.items.map((item, i) => (
                    <motion.div
                      ...
                      whileHover={{ y: -5 }}
                      onClick={() => setSelectedItem({ ...item, category: category.title })}
              ...
              <>
                <DialogHeader>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs font-mono text-primary tracking-wider uppercase bg-primary/10 px-2.5 py-1 rounded">{selectedItem.category}</span>
                  </div>
                  <DialogTitle className="text-2xl font-bold text-foreground">{selectedItem.name}</DialogTitle>
                  <DialogDescription className="text-muted-foreground leading-relaxed">{selectedItem.description}</DialogDescription>
                </DialogHeader>
                <div className="mt-4">
                  <h4 className="text-sm font-semibold text-foreground mb-2">Tools Used</h4>
                  <div className="flex flex-wrap gap-2">
                    {selectedItem.tools.split(", ").map((t) => (
                ...
                >
                  Start a Design Project <ArrowRight className="h-4 w-4" />
                </Link>
              </>
            )}
          </DialogContent>
