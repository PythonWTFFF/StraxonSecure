...
import { Globe, Search, ShieldAlert, Zap, Network, Activity, ShieldCheck, Loader2 } from "lucide-react";
          ...
          <div className="lg:col-span-1 space-y-4">
            <CyberCard variant="plain" className="p-4">
              <form onSubmit={handleAddDomain} className="flex gap-2">
                ...
                >
                  {isAdding ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}
                </button>
              </form>
            </CyberCard>

            <div className="space-y-2">
              <h3 className="text-xs font-mono text-slate-500 uppercase tracking-widest pl-1">Tracked Domains</h3>
              {targets.length === 0 ? (
                <div className="text-xs text-slate-500 font-mono italic p-4 text-center">No targets added yet</div>
              ) : (
                targets.map((t) => (
                  <button
                    key={t.id}
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <Globe className={`h-4 w-4 ${selectedTarget?.id === t.id ? "text-[#00f3ff]" : "text-slate-400"}`} />
                      <span className="font-mono text-sm truncate max-w-[140px] text-slate-200">{t.domain}</span>
                    </div>
                    {t.status === 'scanning' && <Activity className="h-3 w-3 text-yellow-400 animate-pulse" />}
          ...
          <div className="lg:col-span-3">
            {!selectedTarget ? (
              <div className="h-[500px] border border-dashed border-white/10 rounded-xl flex flex-col items-center justify-center text-slate-500 space-y-4">
                ...
                <p className="font-mono text-sm">Select or add a domain to view the attack surface</p>
              </div>
            ) : (
              <div className="space-y-6">
                {/* Target Header */}
                <CyberCard variant={selectedTarget.status === 'scanning' ? 'cyan' : 'plain'} className="p-6">
                    ...
                    <div>
                      ...
                      <p className="text-slate-400 text-sm mt-1 font-mono">
                        Status: <span className={
                          selectedTarget.status === 'scanning' ? 'text-yellow-400 animate-pulse' :
                          selectedTarget.status === 'completed' ? 'text-green-400' : 'text-slate-400'
                        }>{selectedTarget.status.toUpperCase()}</span>
                      </p>
                    </div>
                    <div className="text-right">
                      <div className="text-3xl font-mono text-[#00f3ff]">{findings.length}</div>
                      <div className="text-xs text-slate-500 uppercase tracking-widest">Assets Discovered</div>
                    </div>
                  </div>
                </CyberCard>
                ...
                {isLoadingFindings ? (
                  ...
                  <div className="p-12 text-center text-slate-500 font-mono text-sm border border-dashed border-white/10 rounded-xl">
                    {selectedTarget.status === 'scanning' ? "Recon in progress... discovering assets..." : "No exposed assets found."}
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {findings.map((f) => (
                      <div key={f.id} className="p-4 rounded-lg bg-black/40 border border-white/10 relative group hover:border-white/20 transition-colors">
                        
                        {/* Severity Indicator */}
                        <div className={`absolute top-0 left-0 w-1 h-full rounded-l-lg ${
                          f.severity === 'critical' ? 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]' :
                          f.severity === 'high' ? 'bg-orange-500' :
                          f.severity === 'medium' ? 'bg-yellow-500' :
                          f.severity === 'low' ? 'bg-blue-400' : 'bg-slate-600'
                        }`} />

                        <div className="pl-3 flex flex-col gap-1">
                          <div className="flex items-center gap-2 text-xs font-mono text-slate-400 uppercase">
                            {f.finding_type === 'subdomain' && <Globe className="h-3 w-3 text-blue-400" />}
                            {f.finding_type === 'open_port' && <Zap className="h-3 w-3 text-yellow-400" />}
                            {f.finding_type}
                          </div>
