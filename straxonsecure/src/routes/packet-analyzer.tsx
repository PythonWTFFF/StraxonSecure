...
function PacketAnalyzerPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const mlRes = await fetch(`${mlUrl}/api/analyze-pcap`, {
        method: "POST",
        body: formData,
      });
      const mlData = await mlRes.json();
      if (!mlRes.ok) throw new Error(mlData.detail || "Failed to parse PCAP file");

      const scan = await callAuthed(analyzePacket, {
        filename: file.name,
        size: file.size,
        results: mlData,
      });
  ...
  if (!user)
    return (
      <div className="p-12 text-center text-slate-400 font-mono">
        Sign in to access Packet Analyzer.
      </div>
    );
  ...
  return (
    <div className="px-4 lg:px-8 py-8 max-w-7xl mx-auto space-y-6">
      <SectionHeading
        ...
        description="Upload PCAP payloads for AI-driven threat and anomaly detection."
      />
        ...
        <div className="lg:col-span-1 space-y-6">
          <CyberCard variant="cyan" className="p-4 text-center border-dashed">
            ...
            >
              {analyzing ? (
                ...
                <>
                  <Upload className="h-4 w-4 mr-2" /> Select Payload
                </>
              )}
            </CyberButton>
          </CyberCard>
          ...
          <CyberCard variant="plain" className="p-4">
            <h3 className="text-xs font-mono uppercase tracking-widest text-slate-400 mb-4 flex items-center gap-2">
              <HardDrive className="h-4 w-4" /> Scan History
            </h3>
            {scans.length === 0 ? (
              <div className="text-xs text-slate-400 font-mono italic">No previous scans.</div>
            ) : (
              <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
                {scans.map((s) => (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded border transition-all ${selectedScan?.id === s.id ? "bg-[#00f3ff]/10 border-[#00f3ff]/50" : "bg-white/5 border-white/10 hover:border-white/20"}`}
                  >
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-3 gap-4">
                <CyberCard variant="plain" className="p-4 flex flex-col items-center text-center">
                  ...
                  <span className="text-2xl font-display font-bold text-[#00f3ff] mt-2">
                    {selectedScan.analysis_results.totalPackets}
                  </span>
                </CyberCard>
                <CyberCard variant="plain" className="p-4 flex flex-col items-center text-center">
                  ...
                  <span className="text-2xl font-display font-bold text-red-500 mt-2">
                    {selectedScan.analysis_results.maliciousPackets}
                  </span>
                </CyberCard>
                <CyberCard variant="plain" className="p-4 flex flex-col items-center text-center">
                  <span className="text-xs font-mono text-slate-400 uppercase">Anomalies</span>
                  <span className="text-2xl font-display font-bold text-yellow-500 mt-2">
                    {selectedScan.analysis_results.anomalies}
                  </span>
                </CyberCard>
              </div>

              <CyberCard variant="magenta" className="p-6">
                <h3 className="text-sm font-mono text-[#ff003c] font-bold uppercase mb-4 flex items-center gap-2">
                  <ShieldAlert className="h-4 w-4" /> Threat Signatures Detected
                </h3>
                {selectedScan.analysis_results.detectedThreats.length === 0 ? (
                  <div className="text-green-400 font-mono text-sm">
                    No critical threats detected in payload.
                  </div>
                ) : (
                  <ul className="space-y-2">
                    {selectedScan.analysis_results.detectedThreats.map((t: string, i: number) => (
                      <li
                        key={i}
                        className="flex items-start gap-2 text-sm font-mono text-red-400 bg-red-500/10 p-2 border border-red-500/20 rounded"
                      >
              ...
              <div className="grid grid-cols-2 gap-6">
                <CyberCard variant="plain" className="p-4">
                  <h3 className="text-xs font-mono uppercase text-slate-400 mb-4 border-b border-white/10 pb-2">
                    Top Protocols
                  </h3>
