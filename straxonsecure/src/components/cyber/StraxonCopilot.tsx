...
const TACTICAL_ACTIONS = [
  {
    label: "⚡ Decompile",
    icon: Zap,
    prompt: "Analyze and decompile suspicious base64 payload in telemetry buffer",
  },
  {
    label: "🔍 MITRE TTPs",
    icon: Crosshair,
    prompt: "Map recent perimeter intrusions to MITRE ATT&CK enterprise tactics",
  },
  {
    label: "🛡️ YARA Rule",
    icon: Shield,
    prompt: "Generate an optimized YARA signature rule for the active C2 beacon",
  },
  {
    label: "📊 Blast Radius",
    icon: Terminal,
    prompt: "Calculate lateral movement blast radius if workstation WS-04 is compromised",
  },
];
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Straxon Neural Copilot online. Ready to analyze attack vectors, decompile payloads, or coordinate SOC countermeasures. How can I assist you?",
    },
  ]);
      ...
      >
        ...
        <div className="flex items-center justify-between p-3.5 border-b border-border/50 bg-black/60">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-mono font-bold text-xs tracking-wider text-white">
                STRAXON COPILOT
              </h3>
              <p className="text-[9px] font-mono text-emerald-400">
                DEFCON 2 // NEURAL AGENT ACTIVE
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-primary/20">
          {messages.map((msg, i) => (
            ...
            >
              ...
              <div
                className={`p-3 rounded-xl max-w-[82%] text-xs font-mono leading-relaxed ${msg.role === "user" ? "bg-primary/20 border border-primary/40 text-primary-foreground rounded-tr-none" : "bg-muted/50 border border-border/50 text-foreground rounded-tl-none"}`}
              >
                {msg.content}
              </div>
              {msg.role === "user" && (
